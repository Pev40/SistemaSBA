...
switch($recibo['tipo_inm']){
	case 'P':
		...
		break;
	case 'PA':
		$tipo_rein = 'PlayasAzules';
		break;
	case 'A':
		$tipo_rein = 'Alquileres';
		break;
	case 'TDM':
		$tipo_rein = 'TallerDeMaria';
		break;
	case 'CMC':
		$tipo_rein = 'CementerioRecorrido';
		break;
	case 'PJ':
		$tipo_rein = 'PiscinaJesus';
		break;
}
$tot_saciones = '';
	...
	if($recibo['tipo_inm']=='P'){
		...
		if(isset($item['doc_cliente'])){
			if(strlen($item['doc_cliente']) == 8 && $item['estado']!="X"){
				$tipo_doc = '1';
			}else if(strlen($item['doc_cliente']) == 11 && $item['estado']!="X"){
				$tipo_doc = '6';
			}else if(strlen($item['doc_cliente']) == 9 && $item['estado']!="X"){
				$tipo_doc = '4';
			}
		}else{
			foreach($item['cliente']['docident'] as $doc => $docu){
				if(strlen($docu['num']) == 8 && $item['estado']!="X"){
					$tipo_doc = '1';
				}else if(strlen($item['num']) == 11 && $item['estado']!="X"){
					$tipo_doc = '6';
				}else if(strlen($item['num']) == 9 && $item['estado']!="X"){
					$tipo_doc = '4';
				}
			}
		}
		...
		if($item['serie']=="B001" || $item['serie']=="F001"){
			$serie = $item['serie'];
		}elseif($item['serie']=="B007" || $item['serie']=="F007"){
			$serie = $item['serie'];
		}elseif($item['serie']=="B010" || $item['serie']=="F010"){
				$serie = $item['serie'];
		...
		if($item['tipo'] == 'R'){
			...
			if($item['moneda']=='S' || $item['moneda']=='PEN'){
				if(isset($moras)){
					...
					$objPHPExcel->getActiveSheet()->setCellValue('P'.$row, number_format(floatval($tot_saciones), 2, '.', ''));
				}else{
					$objPHPExcel->getActiveSheet()->setCellValue('L'.$row, number_format($igv, 2, '.', ''));
					$objPHPExcel->getActiveSheet()->setCellValue('J'.$row, number_format($subtotal, 2, '.', ''));
					$objPHPExcel->getActiveSheet()->setCellValue('N'.$row, number_format(($subtotal+$igv), 2, '.', ''));
...
foreach ($rede as $r => $item) {
    $row = $row+1;
    
    // Formato para obtener el nombre completo del cliente
    $cliente = $item['entidad']['nomb'] . ' ' . $item['entidad']['appat'] . ' ' . $item['entidad']['apmat'];
    
    // Calcular subtotales e IGV (Impuesto General a las Ventas)
    $subtotal = $item['efectivos'][0]['monto'] ;
	$tipo = "00";
	$serie = "0000";
    // Asignar valores a las celdas
    $objPHPExcel->getActiveSheet()->setCellValue('A' . $row, date('d/m/Y', $item['fec_db']->sec))
		->setCellValue('B' . $row, $tipo)
		->setCellValue('C' . $row, $serie)
        ->setCellValue('D' . $row, $item['num'])
        ->setCellValue('E' . $row, 1)
        ->setCellValueExplicit('F' . $row, $item['entidad']['docident'][0]['num'], PHPExcel_Cell_DataType::TYPE_STRING)
        ->setCellValue('G' . $row, $cliente)
        ->setCellValue('H' . $row, $item['entidad']['domicilios'][0]['direccion']);

    // Manejar diferentes monedas
    if ($item['moneda'] == 'S' || $item['moneda'] == 'PEN') {
        $objPHPExcel->getActiveSheet()
            ->setCellValue('P' . $row, number_format($subtotal, 2, '.', ''))
            ->setCellValue('Q' . $row, number_format($subtotal, 2, '.', ''));
    }
}
//die();
if($recibo['tipo_inm']=='P'){
