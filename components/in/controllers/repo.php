...
class Controller_in_repo extends Controller {
	...
	function execute_daot(){
		...
		if($tipo_inm_actual == 'MH'){
			$fec = $f->request->data['ano'].'-'.$f->request->data['mes'].'-01';
			$comp = $f->model("cj/comp")->params(array("filter"=>array(
				'modulo' => array('$in' => ['AD', 'MH']), // Filtra por ambos módulos
				'fecreg'=>array(
					'$gte'=>new MongoDate(strtotime($fec)),
					'$lte'=>new MongoDate(strtotime($fec.' +1 month -1 minute'))
				)
	...
	*/
    ...
    function execute_generar_resumen_contingencia_pei(){
		...
		if($comp!=null){
			...
			foreach($comp as $j_ind=>$item){
				...
				$content.="01|";//Tipo de operaci�n
				$content.=date("d/m/Y",$item['fecreg']->sec)."|";//Fecha de emisi�n del comprobante de pago
				$content.=$tipo_docs[$item["tipo"]]."|";//Tipo de comprobante de pago
				$content.=str_pad($item["serie"], 4, "0", STR_PAD_LEFT)."|";//N�mero de serie del comprobante de pago 
				$content.=$item["num"]."|";//N�mero correlativo del comprobante de pago
				$content.="|";//N�mero final del comprobante de pago
				$content.=$tipo_doc."|";//Tipo de documento de identidad del cliente.
				$content.=$num_doc."|";//N�mero de documento de identidad del cliente.
				...
				$content.=number_format(0,2,'.','')."|";//ISC Total operaciones exportaci�n.
				$content.=number_format(0,2,'.','')."|";//ISC 
				...
				$content.="|";//N�mero de serie del comprobante de pago que se modifica.
				$content.="|";//N�mero inicial del comprobante de pago que se modifica.
				$content.="|";//R�gimen de percepci�n.
				$content.="0.00|";//Base imponible de la percepci�n.
				$content.="0.00|";//Monto de la percepci�n.
				$content.="0.00|";//Monto total incluida la percepci�n.
	...
	function execute_continuidad_comprobantes(){
		...
		*	- de lo contrario reporta su numeraci�n
		* - verifica continuidad electr�nica
		...
		*/
		...
		# PORCION DE DIAGNOSTICO DE INFORMATICA, ESCRIBIR EN LA PETICI�N debug O debug=json(FIREFOX)
		if (isset($f->request->data['debug'])) {
			$debug=array(
				'faltantes'=>$faltantes,
        ...
        foreach ($contratos as $cont) {
            ...
            if ($inmu!=null) {
                ...
                if (isset($cont['pagos'])) {
                    foreach ($cont['pagos'] as $pago) {
                        /**
                         * En caso de que la fecha de pago sea menor que año actual (este debiendo)
                         ...
                         *              ['_id']           string  define el _id del comprobante manual o electrónico
                         ...
                         *          ['_id']               MongoId define el _id del comprobante manual o electrónico
