...
*/
	...
	function execute_planilla()
	{
		...
		if ($recibo['tipo_inm'] == 'A') {
		...
		} elseif ($recibo['tipo_inm'] == 'TDM') {
			$inm = array('$in' => array('servicio'));
			#Playas utiliza la serie 004
			$serie = array('$in' => array('B007', 'F007'));
		} elseif ($recibo['tipo_inm'] == 'CMC') {
			$inm = array('$in' => array('agua_chapi', 'servicio', 'pago_meses', 'pago_parcial', 'pago_acta', 'cuenta_cobrar'));
			#Playas utiliza la serie 004
			$serie = array('$in' => array('B012', 'F012'));
		} elseif ($recibo['tipo_inm'] == 'P') {
			$inm = array('$in' => array('servicio'));
			#Playas utiliza la serie 004
			...
			$serie = array('$in' => array('B006', 'F006', 'B013', 'F013'));
		}elseif ($recibo['tipo_inm'] == 'PJ') {
			$inm = array('$in' => array('agua_chapi', 'servicio', 'pago_meses', 'pago_parcial', 'pago_acta', 'cuenta_cobrar'));
			#Playas utiliza la serie 004
			$serie = array('$in' => array('B010', 'F010'));
		}
		$efilter = array(
			'fecemi' => array(
				'$gte' => $recibo['fec'],
		...
		if($recibo['tipo_inm'] == 'A'){
			...
			$rede = $f->model("cj/rede")->params(array("filter"=>$filterRede))->get("all")->items;
			
		}

		$comprobantes = $unicos;
		//$f->response->json(array('recibo' => $recibo, 'comp' => $comprobantes, 'rede' => $rede));
		$f->response->view("in/repo.planilla.print", array('recibo' => $recibo, 'comp' => $comprobantes, 'rede' => $rede));
	...
	function execute_reci_ing2()
	{
		...
		foreach ($recibo['detalle'] as $k => $row) {
			...
			if (is_null($cmpbnt)) {
				$cmpbnt = $f->model("cj/rede")->params(array("_id" => $row['comprobante']['_id']))->get("one")->items;
		...
		foreach ($recibo['detalle'] as $i => $item) {
			...
			if (isset($item['cuenta'])) {
				...
				//var_dump($item);
				//var_dump($detalle_tmp[$item['cuenta']['cod']]);
				//var_dump($item['comprobante']['tipo']);
				//var_dump($item);
				//echo "<br>-------------------------------------<br>";
				if ($item['comprobante']['tipo'] == 'RD') {
					if (!isset($detalle_tmp[$item['cuenta']['cod']]['items']['RD_' . $item['comprobante']['_id']->{'$id'} . $i])) {
						$detalle_tmp[$item['cuenta']['cod']]['items']["RD_" . $item['comprobante']['_id']->{'$id'} . $i] = $item;
					}
					//var_dump($detalle_tmp[$item['cuenta']['cod']]['items']['RD_' . $item['comprobante']['_id']->{'$id'} . $i]);
				} else {
					...
					switch ($recibo['modulo']) {
						case "IN":
							foreach ($comp as $det) {
								...
								if (isset($det['comprobante']['tipo']) && $det['comprobante']['tipo'] == 'RD') {
									//var_dump($det['comprobante']);
									$col_1 = 'RD-'.$det['comprobante']['num'];
									$col_2 = $det['comprobante']['entidad']['nomb'] . ' ' . $det['comprobante']['entidad']['appat'] . ' ' . $det['comprobante']['entidad']['apmat'];
									$col_4 = $det['comprobante']['concepto'];
									$col_6 = number_format($det['monto'], 2);
								} elseif (isset($det['comprobante']['alquiler'])) {
								...
								if (isset($det['comprobante']['tipo_comprobante'])) {
									if ($det['comprobante']['tipo_comprobante'] == "ELECTRONICO") {
												...
												if (isset($det_item['conceptos'])) {
													foreach ($det_item['conceptos'] as $j => $item_conc) {
														...
														if (sizeof($det['comprobante']['items']) == 1 || ($pago_meses + $pago_parcial == 1)) {
															...
															foreach ($det['comprobante']['items'][0]['conceptos'] as $ik => $conc_tmp) {
																if (
																	$conc_tmp['cuenta']['cod'] != '1202.0901.47' &&
										...
										if (isset($det['comprobante']['items'][0]['conceptos'][0]['cuenta_cobrar'])) {
											$cuenta_cobrar = $det['comprobante']['items'][0]['conceptos'][0]['cuenta_cobrar'];
											if (isset($cuenta_cobrar['nomb'])) {
												print_r($cuenta_cobrar['nomb']);
												if (strpos($cuenta_cobrar['nomb'], 'GARANTIA') !== false) {
													$col_5 = "";
												} else {
													$col_5 = $meses[$mes] . ' \'' . $ano;
												}
												print_r("cuenta_cobra" . $col_5 . "----");
											}
										}
									}
								}
		...
		foreach ($recibo['cont_patrimonial'] as $k => $row) {
			$ctasp[$k] = substr($row['cuenta']['cod'], 0, 9); //$row['cuenta']['cod'];
			$ctast[$k] = isset($row['tipo']) ? $row['tipo'] : 0;
		}
		array_multisort($ctast, SORT_ASC, $ctasp, SORT_DESC, $recibo['cont_patrimonial']);
