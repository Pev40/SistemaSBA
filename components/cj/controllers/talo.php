...
class Controller_cj_talo extends Controller
{
    ...
    public function execute_get_talos()
    {
        ...
        // Definir los valores de página
        $pages = array(1, 2);
    
        foreach ($pages as $page) {
            $curl_handle = curl_init();
            $url = 'http://einvoice.conflux.pe/api/v/1/account_einvoice/sequence/?code=' . $toTipo . '&page=' . $page;
            ...
            if ($buffer === false) {
                curl_close($curl_handle);
                $response['message'] = 'El servidor de facturación respondió un contenido vacío, no se pudo obtener los talonarios';
            } else {
                curl_close($curl_handle);
                $result = json_decode($buffer);
