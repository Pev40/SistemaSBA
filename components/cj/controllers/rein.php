...
class Controller_cj_rein extends Controller {
	...
	function execute_get_rec_in(){
		...
		}elseif($f->request->data['tipo_inm']=='TDM'){
			$inm=array('$in' => array('servicio','pago_meses','pago_parcial','pago_acta','cuenta_cobrar'));
			#Playas utiliza la serie 004
			$serie=array('$in' => array('B007','F007'));
		}
		elseif($f->request->data['tipo_inm']=='CMC'){
			$inm=array('$in' => array('servicio','pago_meses','pago_parcial','pago_acta','cuenta_cobrar'));
			#Playas utiliza la serie 004
		...
		}elseif($f->request->data['tipo_inm']=='PJ'){
			$inm=array('$in' => array('servicio'));
			#Playas azules utiliza la serie 006
	...
	function execute_get_rec_ho(){
		...
		$comp = $f->model("cj/comp")->params(array("filter"=>$filter))->get("all")->items;
		$data = array();
		# FILTRAR COMPROBANTES ELECTRONICOS EMITIDOS PARA CAJA DE HERESI
		$efields=array(
			'codigo_barras_pdf'=>0,
			'cliente_email_1'=>0,
			'estado_resumen'=>0,
		...
		$inm=array('$in' => array('servicio','pago_meses','pago_parcial','pago_acta','cuenta_cobrar'));
		#Alquileres utiliza la serie 001
		$serie=array('$in' => array('B008','F008'));
		$efilter = array(
			'fecemi'=>array(
				'$gte'=>new MongoDate(strtotime($f->request->data['fec'])),
				'$lte'=>new MongoDate(strtotime($f->request->data['fecfin'].' +1 day -1 minute'))
