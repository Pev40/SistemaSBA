<?php
class Model_rp_posc extends Model {
	private $db;
	public $items;
	
	public function __construct() {
		global $f;
		$this->db = $f->datastore->re_pos;
	}
	...
	protected function get_search(){
		...
		if($this->params["texto"]!=''){
			...
			$criteria = $helper->paramsSearch($this->params["texto"], array('nomb','serie','salario'));
		}else $criteria = array();
	...
	protected function get_by_caja(){
		global $f;
		$data = $this->db->find(array('caja._id'=>$this->params['caja']));
		foreach ($data as $ob) {
		    $this->items[] = $ob;
		}
	}
