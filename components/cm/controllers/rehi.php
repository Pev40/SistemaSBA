...
class Controller_cm_rehi extends Controller {
	...
	function execute_save_digi() {
		global $f;
		$data = $f->request->data;
		$hist = $f->model("cm/hope")->params(array("_id" => new MongoId($data['_id'])))->get("one")->items;
		if(!isset($hist['url_imagen'])){
			$hist['url_imagen'] = '';
		}
		$hist['url_imagen'] = $data['url_imagen'];
		$model = $f->model("cm/hope")->params(array('_id' => new MongoId($data['_id']), 'data' => $hist))->save("update")->items;
