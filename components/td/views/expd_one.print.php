		...
		if(isset($items["flujos"])){
			...
			if(isset($items["documentos"])){
				...
				foreach($items["documentos"] as $item){
					...
					// Calcular el ancho del texto de "Origen"
					$this->SetFont('arial','',9);
					$origen_text = "Origen: ".$item["organizacion"]["nomb"];
					$origen_width = $this->GetStringWidth($origen_text) + 5; // Añadir un pequeño margen

					$this->SetXY(15,$y);$this->MultiCell($origen_width,5,$origen_text,'1','L');
					$this->SetFont('arial','',7); // Cambiar a tamaño de letra 7
					$foleos_width = 180-$origen_width; // Añadir un pequeño margen

					$this->SetXY($origen_width+15,$y);$this->MultiCell($foleos_width,5,"Nº de Folios: ".$item["folios"],'1','L');
					$this->SetFont('arial','',9); // Volver a tamaño de letra 9
					$y+=5;
